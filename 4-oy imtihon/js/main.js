...
let elMoreForm = document.querySelector(".more_form")
...
let elNewTemplate = document.querySelector("#newTemplate").content;
...
let newTemplate = elNewTemplate.cloneNode(true)
let imageLink = `http://books.google.com/books/content?id=${item.id}&printsec=frontcover&img=1&zoom=5&edge=curl&source=gbs_api`
newTemplate.querySelector(".card-img-top").src = imageLink
newTemplate.querySelector(".card-title").textContent = item.volumeInfo.title
newTemplate.querySelector(".card-author").textContent = item.volumeInfo.authors
newTemplate.querySelector(".card-year").textContent = item .volumeInfo.publishedDate
...
elNewBook.addEventListener("click", (evt) =>{

let foundUser = evt.target.dataset.bookId;
